...
public class GestureDetectorImageView extends ImageView implements OnScaleGestureListener, OnTouchListener,
		ViewTreeObserver.OnGlobalLayoutListener
{
		...
		mGestureDetector = new GestureDetector(context, new SimpleOnGestureListener() {
			...
			public boolean onDoubleTap(MotionEvent e) {
				...
				if (getScale() < SCALE_MID) {
					GestureDetectorImageView.this.postDelayed(new AutoScaleRunnable(SCALE_MID, x, y), 16);
					isAutoScale = true;
				} else {
					GestureDetectorImageView.this.postDelayed(new AutoScaleRunnable(initScale, x, y), 16);
					isAutoScale = true;
				}
	...
	private class AutoScaleRunnable implements Runnable {
		...
		public void run() {
			...
			if (((tmpScale > 1f) && (currentScale < mTargetScale))
				...
				GestureDetectorImageView.this.postDelayed(this, 16);
			} else
			// 设置为目标的缩放比例
			{
	...
	/**
	 * 剪切圆形图片，返回剪切后的bitmap对象
	 * 
	 * @return
	 */
	public Bitmap cutCircle() {
		Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(), Config.ARGB_8888);
		Canvas canvas = new Canvas(bitmap);
		draw(canvas);
		return getCircleBitmap(Bitmap.createBitmap(bitmap, mHorizontalPadding, mVerticalPadding, getWidth() - 2
